  ...
  <div class="app-container">
    ...
    >
      <el-table-column
        ...
        width="40"
      />
      <el-table-column label="设备ID" sortable="custom" align="center" show-overflow-tooltip>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="开始日期" prop="date_start">
          <el-date-picker v-model="temp.date_start" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
        <el-form-item label="截止日期" prop="date_end">
          <el-date-picker v-model="temp.date_end" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
...
<script>
import { getGatewayPage, updateGateway, addGateway, deleteGateway, batchDeleteGateways } from '@/api/gateway'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getGatewayPage(this.listQuery).then((response) => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addGateway(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateGateway(tempData).then((response) => {
            this.dialogFormVisible = false
            const { status } = response
    ...
    handleDelete(row, index) {
      this.$confirm('确定要删除吗？').then(() => {
        deleteGateway(row.device_id).then((response) => {
          const { status } = response
    ...
    handleBatchDelete() {
      ...
      this.$confirm('确定要删除吗？').then(() => {
        ...
        batchDeleteGateways(ids).then((response) => {
          const { status } = response
