<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <el-image :src="row.imgStorageUrl" lazy> </el-image>
        </template>
      </el-table-column>
      <el-table-column
        label="图片点赞数"
      ...
      >
        <template slot-scope="{ row }">
          <span>{{ row.imgFont }}</span>
        </template>
      </el-table-column>

      <el-table-column label="创建日期" align="center" show-overflow-tooltip>
        <template slot-scope="{ row }">
          <span>{{ row.imgCreateTime | moment("YYYY-MM-DD ") }}</span>
        </template>
      </el-table-column>
...
<script>
import { getDiscipleAcquire, getDiscipleList, getUser } from "@/api/user";
import Vue from "vue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getUser(nodename).then((response) => {
